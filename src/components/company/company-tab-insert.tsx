...
const CompanyTabInsert: React.FC<Props> = ({}) => {
  ...
  const onFinish = async (values: any) => {
    ...
    const companyInfo: CompanyInfo = {
      ...
      initial_listing_volume: values.initialListedVolume || null,
      current_listing_volume: values.currentListedVolume || null,
    ...
    try {
      // Lấy ID trực tiếp từ UpsertCompany
      const companyId = await api.UpsertCompany(companyInfo, dispatch);

      // Sử dụng company_id để cập nhật các Stakeholder
      const stakeholdersWithCompanyId = boardMembers.map((member) => ({
        ...member,
          ...
          <div className="space-y-4">
            ...
            >
              <Select>
                ...
                <Option value="Công ty trách nhiệm hữu hạn">
              ...
              <Upload
                ...
                onChange={(info) => {
                  if (info.file.status === "done") {
                    console.log("File uploaded:", info.file.response?.url); // URL trả về từ server
                  }
                }}
              >
            ...
            <Form.Item
              label="Website côn ty"
              name="companyWebsite"
              rules={[
                {
                  required: true,
                  message: "Vui lòng nhập đường dẫn đến Website công ty!",
                },
              ]}
            >
              <Input />
